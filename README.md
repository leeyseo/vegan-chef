...
- 📸 **냉장고 스캔** — 사진 업로드(카메라/앨범) → AI 비전 모델이 식물성 재료를 인식
- ⚡ **실시간 분석** — 인식한 재료가 하나씩 뜨고, 레시피 생성 진행률을 실시간 표시(SSE 토큰 스트리밍)
- 🥗 **비건 레시피 매칭** — 보유 재료 충족도(매치율)순 정렬, `글루텐프리 / 퀵 / 고단백` 필터
- 📖 **레시피 상세** — 재료 체크리스트 · 단계별 조리법 · 에코 임팩트(절감 탄소·물) 탭
- 💾 **기기 저장** — 같은 기기로 재방문하면 지난 분석을 이어볼 수 있음(localStorage)
...
| AI | **Google Gemini 2.5 Flash**(무료 티어, OpenAI 호환 API) — 이미지 입력 + 토큰 스트리밍 JSON |
...
```
┌────────────┐  사진(base64)   ┌──────────────┐  이미지+프롬프트   ┌──────────────┐
│ React SPA  │ ──────────────▶ │ Express 프록시 │ ───────────────▶ │ Gemini 2.5   │
│ (Vite)     │ ◀──SSE 스트림── │  (:3001)      │ ◀──토큰 스트림── │ Flash (비전) │
└────────────┘  진행+레시피      └──────────────┘                  └──────────────┘
```

- 분석은 **스트리밍**: 모델이 토큰을 흘려보내면 서버가 인식 재료·레시피 수·진행률을 SSE로 실시간 전달.
...
```bash
npm install
cp .env.example .env      # GEMINI_API_KEY 입력 (무료: https://aistudio.google.com)
...
| 변수 | 기본값 | 설명 |
|------|--------|------|
| `GEMINI_API_KEY` | — | **(필수)** Google Gemini API 키 (무료, aistudio.google.com) |
| `GEMINI_MODEL` | `gemini-2.5-flash` | 사용 모델 (품질↑: `gemini-2.5-pro` / 더 빠름: `gemini-2.0-flash`) |
| `PORT` | `3001` | 백엔드 포트 (Cloud Run은 자동 주입) |
| `MAX_OUTPUT_TOKENS` | `8000` | 요청당 출력 토큰 상한 |
...
> 모델 교체가 쉽도록 **OpenAI 호환 클라이언트**를 사용합니다. `server/index.js`에서 `baseURL`·키·모델만 바꾸면 Groq(오픈 Llama) 등 다른 OpenAI 호환 제공자로도 전환할 수 있습니다.

## 💸 비용 · 남용 보호

- **사실상 무료** — Gemini 무료 티어로 분석 호출 비용이 0원(무료 한도 내). 호스팅(Cloud Run)도 scale-to-zero라 이 규모에선 사실상 $0.
- **앱 레벨 가드** — IP별 시간/일 + 전체 일일 요청 상한 초과 시 `429`, 요청당 출력 토큰 상한.
- **한도 초과 시** — 무료 티어 분당/일일 한도를 넘으면 일시적으로 `429`. 한도는 [aistudio.google.com](https://aistudio.google.com)에서 확인.
