...
// 스트리밍용 지시문 — output_config(구조화 출력)는 JSON을 끝에 한 번에 주므로
// 토큰 단위 스트리밍이 안 된다. 대신 프롬프트로 동일한 JSON 구조를 강제해, 일반 텍스트
// 스트리밍(토큰 단위)으로 받으면서 진행 상황을 실시간으로 추출한다.
const STREAM_JSON_INSTRUCTION = `이 사진을 분석해 식물성 재료를 파악하고, 동물성 재료 없이 만들 수 있는 비건 레시피를 4개 추천해줘.
...
app.post("/api/analyze/stream", rateLimit, async (req, res) => {
  ...
  try {
    const stream = client.messages.stream({
      ...
      max_tokens: MAX_OUTPUT_TOKENS,
      system: SYSTEM_PROMPT,
      // output_config는 일부러 생략 — 토큰 단위 스트리밍을 위해 프롬프트로 JSON을 강제한다.
      messages: [
        {
          role: "user",
          content: [
            ...
            { type: "text", text: STREAM_JSON_INSTRUCTION },
          ],
        },
      ],
    });
