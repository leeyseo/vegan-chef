...
if (!apiKey) {
  console.warn(
    "\n⚠️  GROQ_API_KEY 가 설정되지 않았습니다.\n" +
      "   https://console.groq.com 에서 무료 키를 발급받아 .env 에 넣어주세요 (.env.example 참고).\n"
  );
}

const client = new Groq({ apiKey });
// 오픈소스 Llama 비전 모델을 Groq 무료 호스팅으로 사용. 모델 ID는 가끔 바뀌므로
// GROQ_MODEL 로 교체 가능. 현재 후보: meta-llama/llama-4-scout-17b-16e-instruct,
// meta-llama/llama-4-maverick-17b-128e-instruct (목록: https://console.groq.com/docs/models)
const MODEL = process.env.GROQ_MODEL || "meta-llama/llama-4-scout-17b-16e-instruct";
// 요청당 출력 토큰 상한. (Groq 무료 모델은 보통 8K 내외)
const MAX_OUTPUT_TOKENS = Number(process.env.MAX_OUTPUT_TOKENS ?? 8000);

/* ──────────────────────────────────────────────────────────────
   비용/남용 보호: IP별 속도 제한 + 전역 일일 상한
   - Groq 무료 티어에도 분당/일일 한도가 있으므로, 폭주를 막기 위해 요청 수를
...
const JSON_INSTRUCTION = `이 사진을 분석해 식물성 재료를 파악하고, 동물성 재료 없이 만들 수 있는 비건 레시피를 4개 추천해줘.
...
// 이미지 + 지시문을 OpenAI 호환(Groq) 메시지 형식으로 구성.
function buildMessages(imageBase64, mediaType) {
...
app.post("/api/analyze", rateLimit, async (req, res) => {
  try {
    if (!apiKey) {
      return res
        .status(500)
        .json({ error: "서버에 GROQ_API_KEY 가 설정되지 않았습니다." });
    }
